...
export class ConnectionsService {
  ...
  async sendIntroductions(userId: string, introductionDto: IntroductionDto): Promise<{ message: string; emailsSent: number }> {
    // Get seller profile and user details
    const seller = await this.sellerModel.findOne({ userId }).populate('userId');
    if (!seller) {
    ...
    const invalidIds = introductionDto.advisorIds.filter(id => !matchedIds.includes(id));
    
    if (invalidIds.length > 0) {
      throw new BadRequestException('Some advisor IDs are not from your current matches');
    }

    // Get selected advisors with user details
    const selectedAdvisors = await this.advisorModel.find({
    ...
    // Load email template
    const templatePath = path.join(process.cwd(), 'templates', 'introduction.hbs');
    let template = '';
    try {
    ...
    for (const advisor of selectedAdvisors) {
      ...
      const emailHtml = template
        ...
        .replace(/{{sellerDescription}}/g, seller.description || 'No description provided')
  ...
  async sendDirectContactList(userId: string): Promise<{ message: string; advisorCount: number }> {
    // Get seller profile and user details
    const seller = await this.sellerModel.findOne({ userId }).populate('userId');
    if (!seller) {
      throw new NotFoundException('Seller profile not found');
    ...
    const advisors = await this.advisorModel.find({
      _id: { $in: matches.map(m => m.id) }
    }).populate('userId');

    // Load templates
    const directListTemplate = fs.readFileSync(
      path.join(process.cwd(), 'templates', 'direct-list.hbs'), 'utf8'
    );
    const notificationTemplate = fs.readFileSync(
      path.join(process.cwd(), 'templates', 'match-notification.hbs'), 'utf8'
    );
    ...
    const advisorListHtml = advisorData.map(advisor => 
    ...
    if (loopStart !== -1 && loopEnd !== -1) {
      listEmailHtml = listEmailHtml.substring(0, loopStart) + advisorListHtml + listEmailHtml.substring(loopEnd);
    }
    ...
    for (const advisor of advisors) {
      ...
      try {
        ...
        console.error(`Failed to send notification to ${advisorUser.email}:`, error);
      }
    }
